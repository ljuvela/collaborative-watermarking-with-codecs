...
if __name__ == "__main__":
    ...
    # clean_vits_filelist(vits_filelist_train, os.path.join(args.target_dir, 'ljspeech_filelist_train_text.txt'), args.prefix, remove_text=False)
    # clean_vits_filelist(vits_filelist_val, os.path.join(args.target_dir, 'ljspeech_filelist_val_text.txt'), args.prefix, remove_text=False)
    # clean_vits_filelist(vits_filelist_test, os.path.join(args.target_dir, 'ljspeech_filelist_test_text.txt'), args.prefix, remove_text=False)

    # Keep text and append prefix (local dirs)
    clean_vits_filelist(
        vits_filelist_train,
        os.path.join(args.target_dir, "ljspeech_filelist_train_text_local.txt"),
        prefix="/tmp/ljuvela/data/ljspeech/train",
        remove_text=False,
    )
    clean_vits_filelist(
        vits_filelist_val,
        os.path.join(args.target_dir, "ljspeech_filelist_val_text_local.txt"),
        prefix="/tmp/ljuvela/data/ljspeech/val",
        remove_text=False,
    )
    clean_vits_filelist(
        vits_filelist_test,
        os.path.join(args.target_dir, "ljspeech_filelist_test_text_local.txt"),
        prefix="/tmp/ljuvela/data/ljspeech/test",
        remove_text=False,
    )

    # Split text and remove prefix
    # clean_vits_filelist(vits_filelist_train, os.path.join(args.target_dir, 'ljspeech_filelist_train.txt'))
    # clean_vits_filelist(vits_filelist_val, os.path.join(args.target_dir, 'ljspeech_filelist_val.txt'))
    # clean_vits_filelist(vits_filelist_test, os.path.join(args.target_dir, 'ljspeech_filelist_test.txt'))
