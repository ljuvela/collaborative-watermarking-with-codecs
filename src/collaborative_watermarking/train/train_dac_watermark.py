...
from collaborative_watermarking.losses import WatermarkLoss
from collaborative_watermarking.metrics import WatermarkEER
...
Watermark = argbind.bind(WatermarkModel)
...
WatermarkLoss = argbind.bind(WatermarkLoss)
...
def load(
    ...
    if resume:
        ...
        if (Path(kwargs["folder"]) / "watermark").exists():
            watermark, w_extra = WatermarkModel.load_from_folder(
                folder=f"{save_path}/{tag}",
                map_location="cpu",
                package=False,
            )
    ...
    watermark = Watermark(args['Watermark.model_type'], args['Watermark.sample_rate'], args['Watermark.config']) if watermark is None else watermark

    tracker.print(generator)
    tracker.print(discriminator)
    tracker.print(watermark)

    generator = accel.prepare_model(generator)
    discriminator = accel.prepare_model(discriminator)
    watermark = accel.prepare_model(watermark)
    ...
    with argbind.scope(args, "watermark"):
        optimizer_wm = AdamW(watermark.parameters(), use_zero=accel.use_ddp)
        scheduler_wm = ExponentialLR(optimizer_wm)
    ...
    watermark_loss = WatermarkLoss(detector=watermark, mode=args['Watermark.mode'])
    watermark_eer = WatermarkEER(watermark)
...
def val_loop(batch, state, accel):
    ...
    return {
        ...
        "watermark/eer": state.watermark_eer(real=signal.audio_data, fake=recons.audio_data),
    }
...
def train_loop(state, batch, accel, lambdas):
    ...
    with accel.autocast():
        ...
        output["watermark/loss"] = state.watermark_loss(fake=recons.audio_data, real=signal.audio_data)
        output["loss"] = sum([v * output[k] for k, v in lambdas.items() if k in output])

    state.optimizer_g.zero_grad()
    state.optimizer_wm.zero_grad()
    accel.backward(output["loss"])
    accel.scaler.unscale_(state.optimizer_g)
    output["other/grad_norm"] = torch.nn.utils.clip_grad_norm_(
        state.generator.parameters(), 1e3
    )
    accel.scaler.unscale_(state.optimizer_wm)
    output["other/grad_norm_watermark"] = torch.nn.utils.clip_grad_norm_(
        state.watermark.parameters(), 1e3
    )
    accel.step(state.optimizer_g)
    state.scheduler_g.step()
    accel.step(state.optimizer_wm)
    state.scheduler_wm.step()
    accel.update()
...
def checkpoint(state, save_iters, save_path):
    ...
    for tag in tags:
        ...
        watermark_extra = {
            "optimizer.pth": state.optimizer_wm.state_dict(),
            "scheduler.pth": state.scheduler_wm.state_dict(),
        }
        accel.unwrap(state.watermark).save_to_folder(
            folder=f"{save_path}/{tag}",
            extra_data=watermark_extra,
            package=False,
        )
