...
def train(rank, a, h):
    ...
    # check for pretrained path in arguments
    if a.pretrained_hifigan_path is not None:
        cp_g = scan_checkpoint(a.pretrained_hifigan_path , 'g_')
        cp_do = scan_checkpoint(a.pretrained_hifigan_path , 'do_')
        cp_wm = scan_checkpoint(a.pretrained_hifigan_path , 'wm_')
    elif os.path.isdir(a.checkpoint_path):
    ...
    training_filelist, validation_filelist = get_dataset_filelist(a, ext=a.wavefile_ext)
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss_wm, loss_r_wm, loss_f_wm = discriminator_loss(
            ...
            if rank == 0:
                # STDOUT logging
                if steps % a.stdout_interval == 0 or force_logging:
                    with torch.no_grad():
                ...
                if steps % a.summary_interval == 0 or force_logging:
                    ...
                    if a.log_training_eer:
                        watermark_metrics = DiscriminatorMetrics()
                        with torch.no_grad():
                            watermark_metrics.accumulate(
                                disc_real_outputs = [y_wm_real],
                                disc_fake_outputs = [y_wm_fake]
                            )
...
def main():
    ...
    parser.add_argument('--pretrained_hifigan_path', default=None)
